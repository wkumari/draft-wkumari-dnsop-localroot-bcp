...
informative:
  ...
  Locally-Served:
    title: IANA Locally-Served DNS Zones Registry
...
1. Identify locations from where IANA root zone data can be obtained
   ({{root-zone-sources}}).
2. Download and refresh the IANA root zone data from one of the
   publication points ({{protocol-steps}}).
3. Integrating and serving the IANA root zone data while performing DNS
   resolutions ({{integrating-root-zone-data}}).
...
## Identifying locations from where IANA root zone data can be obtained {#root-zone-sources}
...
## Downloading and refreshing IANA root zone data {#protocol-steps}

Once a list of available publication points of IANA root zone data
is configured or obtained, a LocalRoot implementation MAY
...
3. If the LocalRoot implementation fails to retrieve the IANA root
  ...
  data and SHOULD wait one refresh interval before retrying
  sources.

1. After successfully downloading a copy of the IANA root zone, the
...
1. The resolver MUST check at least one of the sources in step 1 at a
...
## Integrating and serving the IANA root zone data during resolution {#integrating-root-zone-data}
...
### Pre-caching the IANA root zone data
...
### Running a local authoritative copy of the IANA root zone in parallel
...
data related to the IANA root zone instead of the RSS itself.
...
### Zone loaded and served by Recursive Resolvers

Recursive resolvers may load authoritative zones to be used when constructing
query responses to clients, for example to serve the {{Locally-Served}} zones.
This approach is taken by the BIND 9 Mirror Zones {{BIND-MIRROR}} feature, and
the zone is updated using normal secondary zone update processing.
...
- A LocalRoot implementation MUST have a configured DNSSEC trust
  ...
  Key (KSK) {{RFC4033}} or used to sign the IANA root or its DS record.
...
- A LocalRoot implementation MUST validate the contents of the IANA root zone
  using ZONEMD {{RFC8976}}, and MUST check the validity of the ZONEMD record
  using DNSSEC.

- A LocalRoot implementation MUST use and serve records from the IANA root
...
- A LocalRoot implementation MUST have an upper time limit beyond
  ...
  SOA expiry value.  It MUST NOT be longer than the IANA root zone's
...
This document differs in a number of critical ways (TBD: this list is
incomplete):
